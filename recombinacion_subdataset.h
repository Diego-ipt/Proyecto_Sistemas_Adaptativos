...
pair<vector<string>, vector<double>> init_subdatasets_solutions(const vector<string>& dataset, int num_subdatasets, int threshold,int max_error, 
...
unordered_map<int, string> index_to_substring){
    ...
    pair<vector<string>, vector<double>> soluciones_subdatasets;
    for(int i = 0; i < num_subdatasets; i++){
        solutionss individuo=init_solucion_SuDa(subdataset[i].dataset, threshold, max_error, temperature_pert, temperature_leap, cooling_rate, heat_rate, iteraciones_max, substring_to_index, index_to_substring);
        soluciones_subdatasets.first.push_back(individuo.solucion);
        soluciones_subdatasets.second.push_back(individuo.calidad);
    }
...
unsigned X_INTVL, unsigned X_NUMBER, bool tuningMode){
    ...
    pair<vector<string>, vector<double>> soluciones_subdatasets = init_subdatasets_solutions(dataset, num_subdatasets, threshold, max_error, temperature_pert, temperature_leap, cooling_rate, heat_rate, iteraciones_max, substring_to_index, index_to_substring);
    ...
    algorithm.pushIndividuals_AGTC(soluciones_subdatasets.first, soluciones_subdatasets.second);
